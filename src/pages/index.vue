...
<template>
    <div>
        ...
        <div ref="myEl" class="mx-2 my-4 py-4 border-2 border-blue-900 rounded-lg text-white relative">
            <h1 class="text-2xl mb-7 text-uppercase font-semibold">Strat Of The Month: <router-link to="/strat/2501" class="text-green-500">{{ stratname }}</router-link></h1>
  ...
  setup: () => {
    ...
    const myEl = ref(null);
     ...
     const getStratData = () => {
        ...
        return axios.get('/api/stratdata?id=2501')
    }
    ...
    onMounted( async () => {
        axios.get('/api/strategy?id=2501')
        .then( s => {

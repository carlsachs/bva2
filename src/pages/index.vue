<template>
    <div>
        ...
        <div class="p-4 grid sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-2 gap-1 sm:gap-5">
            <div v-for="(strategy, i) in strategies" :key="strategy.stratname" class="mx-2 my-4 py-4 border-2 border-blue-900 rounded-lg text-white relative">
                
                <router-link :to="'/strat/'+strategy.stratid" class=""><h1 class="mb-0 text-2xl text-uppercase font-semibold">{{ strategy.stratname }}</h1></router-link>
                ...
                <div v-if="!auth0.state.isAuthenticated" @click="login" class="mx-10 p-4 bg-indigo-900 text-orange-500 font-bold group flex-auto items-center shadow-xl gap-5 rounded-lg mt-5 cursor-pointer hover:bg-blue-900 hover:bg-opacity-100 transition">
                    Limited Availability {{ strategy.price }} 
                </div>
                <div class="p-4 grid sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-2 gap-1 sm:gap-5 uppercase">
                    ...
                    <router-link :to="'/strat/'+strategy.stratid" class="group bg-indigo-900 flex items-center bg-opacity-40 shadow-xl gap-5 px-6 py-5 rounded-lg mt-5 cursor-pointer hover:bg-blue-900 hover:bg-opacity-100 transition">
...
import { onMounted, reactive, computed, toRefs, watch, defineComponent, inject } from "vue"
...
export default defineComponent({
  ...
  setup: () => {
    ...
    watch( () => products?.items, (prods) => {
        console.log("======-=-=-=55555=-=-======")
        prods.map( yo => {
            console.log(yo.name)
            yo.stratid = yo.stratid
            yo.stratname = yo.name
            yo.description = yo.description
            yo.max_concurrent = yo.max_concurrent
            yo.price = yo.price + " " + yo.currency
            yo.series = [{ name: "", data: [] }]
            yo.total_pnl = 0
            yo.total_signals = 0
            yo.avg_pnl = 0
        ...
        state.strategies = prods
        for ( const i in state.strategies ) {
            axios.get('/api/stratdata?id=' + state.strategies[i].stratid)
            .then( rows => {
                let tpnl_bva = []
                let pnl_bva = 0
    ...
    onMounted(() => {
        ...
        state.strategies = products.items
        for ( const i in state.strategies ) {
            axios.get('/api/stratdata?id=' + state.strategies[i].stratid)
            .then( rows => {
                let tpnl_bva = []

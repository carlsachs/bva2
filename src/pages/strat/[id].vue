                    ...
                    if (type === 'LONG') {
                        ...
                        if (sum_sell_trade_size) {
                            if (row?.sell_trade_size_4) {
                                ...
                                const pnl_1 = sell_price_1.minus(dca_buy_price).times(row.sell_trade_size).dividedBy(dca_buy_price).dividedBy(sum_buy_trade_size)
                                const pnl_2 = sell_price_2.minus(dca_buy_price).times(row.sell_trade_size_2).dividedBy(dca_buy_price).dividedBy(sum_buy_trade_size)
                                const pnl_3 = sell_price_3.minus(dca_buy_price).times(row.sell_trade_size_3).dividedBy(dca_buy_price).dividedBy(sum_buy_trade_size)
                                const pnl_4 = sell_price_4.minus(dca_buy_price).times(row.sell_trade_size_4).dividedBy(dca_buy_price).dividedBy(sum_buy_trade_size)
                        ...
                        if (sum_buy_trade_size) {
                            if (row?.buy_trade_size_4) {
                                ...
                                const pnl_1 = dca_sell_price.minus(buy_price_1).times(row.buy_trade_size).dividedBy(buy_price_1).dividedBy(sum_sell_trade_size)
                                const pnl_2 = dca_sell_price.minus(buy_price_2).times(row.buy_trade_size_2).dividedBy(buy_price_2).dividedBy(sum_sell_trade_size)
                                const pnl_3 = dca_sell_price.minus(buy_price_3).times(row.buy_trade_size_3).dividedBy(buy_price_3).dividedBy(sum_sell_trade_size)
                                const pnl_4 = dca_sell_price.minus(buy_price_4).times(row.buy_trade_size_4).dividedBy(buy_price_4).dividedBy(sum_sell_trade_size)
                                const pnl_5 = dca_sell_price.minus(last_price).times(100 - sum_buy_trade_size).dividedBy(last_price).dividedBy(sum_sell_trade_size)
                                pnl = Number(pnl_1.plus(pnl_2).plus(pnl_3).plus(pnl_4).plus(pnl_5).times(100).minus(0.2).toFixed(2))
                            }
                                ...
                                const pnl_2 = dca_sell_price.minus(buy_price_2).times(row.buy_trade_size_2).dividedBy(buy_price_2).dividedBy(sum_sell_trade_size)
                                const pnl_3 = dca_sell_price.minus(buy_price_3).times(row.buy_trade_size_3).dividedBy(buy_price_3).dividedBy(sum_sell_trade_size)
                                const pnl_4 = dca_sell_price.minus(last_price).times(100 - sum_buy_trade_size).dividedBy(last_price).dividedBy(sum_sell_trade_size)
                                pnl = Number(pnl_1.plus(pnl_2).plus(pnl_3).plus(pnl_4).times(100).minus(0.2).toFixed(2))
                            }
                                ...
                                const pnl_1 = dca_sell_price.minus(buy_price_1).times(row.buy_trade_size).dividedBy(buy_price_1).dividedBy(sum_sell_trade_size)
                                const pnl_2 = dca_sell_price.minus(buy_price_2).times(row.buy_trade_size_2).dividedBy(buy_price_2).dividedBy(sum_sell_trade_size)
                                const pnl_3 = dca_sell_price.minus(last_price).times(100 - sum_buy_trade_size).dividedBy(last_price).dividedBy(sum_sell_trade_size)
                                pnl = Number(pnl_1.plus(pnl_2).plus(pnl_3).times(100).minus(0.2).toFixed(2))
                            }
                            else if (row?.buy_trade_size) {
                                const buy_price_1 = new BigNumber(row.buy_price)
                                const pnl_1 = dca_sell_price.minus(buy_price_1).times(row.buy_trade_size).dividedBy(buy_price_1).dividedBy(sum_sell_trade_size)
                                const pnl_2 = dca_sell_price.minus(last_price).times(100 - sum_buy_trade_size).dividedBy(last_price).dividedBy(sum_sell_trade_size)
                                pnl = Number(pnl_1.plus(pnl_2).times(100).minus(0.2).toFixed(2))
                            }
                        }
                        else {
